...
<body class="bg-white text-gray-900 font-sans antialiased transition-colors duration-300">
    ...
    <nav class="p-6 sticky top-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md z-50 border-b border-gray-100 dark:border-gray-800">
    ...
    <main class="max-w-5xl mx-auto py-12 px-6">
        <h1 class="text-4xl md:text-5xl font-serif font-bold mb-8 leading-tight dark:text-white">{{ $project->title }}</h1>
            ...
            <div class="mb-12 relative group rounded-xl overflow-hidden bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-gray-700 shadow-sm"
                ...
                <div class="relative w-full aspect-[4/3] md:aspect-[16/9]">
                    @foreach($project->image as $index => $img)
                        <div x-show="activeSlide === {{ $index }}" class="absolute inset-0 w-full h-full flex items-center justify-center bg-gray-100 dark:bg-slate-700">
                             <img src="{{ $img['url'] }}" class="w-full h-full object-contain" alt="Project Image">
                        </div>
                    @endforeach
                </div>
                @if($count > 1)
                    <button @click="activeSlide = activeSlide === 0 ? total - 1 : activeSlide - 1" class="absolute left-4 top-1/2 -translate-y-1/2 bg-white text-black w-10 h-10 flex items-center justify-center rounded-full shadow-md">&larr;</button>
                    <button @click="activeSlide = activeSlide === total - 1 ? 0 : activeSlide + 1" class="absolute right-4 top-1/2 -translate-y-1/2 bg-white text-black w-10 h-10 flex items-center justify-center rounded-full shadow-md">&rarr;</button>
                @endif
            </div>
        @endif
